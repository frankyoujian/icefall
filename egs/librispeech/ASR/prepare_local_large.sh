...
dev_test_sets="dev-gigaspeech dev-peoplespeech dev-voxpopuli test-gigaspeech test-peoplespeech test-voxpopuli"
# dev_test_sets="test_wav_flac"
train_sets="train-gigaspeech train-peoplespeech train-voxpopuli"
# train_sets=""
all_sets="${dev_test_sets} ${train_sets}"
...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
  ...
  if [ ! -e data/manifests/.librispeech.done ]; then
    ./local/prepare_local_data.py $dl_dir/LibriSpeech data/manifests "${all_sets}" $nj
...
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
  log "Stage 3: Compute fbank for ${dev_test_sets}"
  mkdir -p data/fbank
  if [ ! -e data/fbank/.librispeech.dev.test.done ]; then
    ./local/compute_fbank_local_data.py --dataset "${dev_test_sets}"
    touch data/fbank/.librispeech.dev.test.done
  fi
fi
...
if [ $stage -le 4 ] && [ $stop_stage -ge 4 ]; then
  log "Stage 4: Split ${train_sets}"
  for subset in ${train_sets}; do
    log "Spliting subset : $subset"
    split_dir=${manifests_dir}/librispeech_${subset}_split
    mkdir -p ${split_dir}
    if [ ! -e ${split_dir}/.split_completed ]; then
      lhotse split-lazy ${manifests_dir}/librispeech_cuts_${subset}.jsonl.gz ${split_dir} ${num_per_split}
...
if [ $stage -le 5 ] && [ $stop_stage -ge 5 ]; then
  log "Stage 5: Compute fbank for ${train_sets}"
  mkdir -p data/fbank
  for subset in ${train_sets}; do
    num_splits=$(find ${manifests_dir}/librispeech_${subset}_split -name "librispeech_cuts_${subset}.*.jsonl.gz" | wc -l)
    if [ ! -e data/fbank/.librispeech.${subset}.done ]; then
        ./local/compute_fbank_local_data.py \
          --dataset ${subset} \
          --num-splits ${num_splits}
      touch data/fbank/.librispeech.${subset}.done
    fi
  done
fi

if [ $stage -le 6 ] && [ $stop_stage -ge 6 ]; then
  log "Stage 6: Combine features for ${train_sets}"
  for subset in ${train_sets}; do
    log "Combining $subset subset."
    if [ ! -f $fbank_dir/librispeech_cuts_${subset}.jsonl.gz ]; then
      pieces=$(find ${fbank_dir}/librispeech_${subset}_split -name "librispeech_cuts_${subset}.*.jsonl.gz")
      lhotse combine $pieces ${fbank_dir}/librispeech_cuts_${subset}.jsonl.gz
    fi
  done
fi

if [ $stage -le 7 ] && [ $stop_stage -ge 7 ]; then
  log "Stage 7: Validating data/fbank for LibriSpeech"
  if [ ! -f data/fbank/librispeech_cuts_train-all-shuf.jsonl.gz ]; then
    cat <(gunzip -c data/fbank/librispeech_cuts_train-gigaspeech.jsonl.gz) \
      <(gunzip -c data/fbank/librispeech_cuts_train-peoplespeech.jsonl.gz) \
      <(gunzip -c data/fbank/librispeech_cuts_train-voxpopuli.jsonl.gz) | \
...
if [ $stage -le 9 ] && [ $stop_stage -ge 9 ]; then
  ...
  for vocab_size in ${vocab_sizes[@]}; do
    ...
    if [ ! -f $lang_dir/transcript_words.txt ]; then
      ...
      find ${dl_dir}/LibriSpeech -path "*/train*/datalist.json" -print0 | xargs -0 jq -r '.[].text' > ${lang_dir}/transcript_words.txt
    fi
